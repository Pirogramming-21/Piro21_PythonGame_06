...
def current_status():
    ...
    for i in range(len(players)):
        print(f'{players[i][0]}은(는) 지금까지 {players[i][2]}🍺! 치사량까지 {players[i][1]}')
    line_print()
...
def playing_game(game_index):
    ...
    game_loser = game_to_play()
    print(f'아 누가누가 술을 마셔😮 {game_loser[0]}이(가) 술을 마셔😖 원~~~샷✨✨')
    drink_soju(game_loser)

# 게임 선택
game_index = random_game_player(players[0])

# 게임 실행
games = [game_1, game_2, game_3, game_4, game_5]
playing_game(game_index)
